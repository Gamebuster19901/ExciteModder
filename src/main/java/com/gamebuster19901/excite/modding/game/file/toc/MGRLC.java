		...
		while(compressed.position() < compressed.limit()) {
			...
			for(int i = 0; i < 31; i++) { //for each bit in 32 bit header, except the last one
				if((header & (1 << i)) == 0) { //if bit at position i is 0
					...
					decompressed.put(decompressed.get(new byte[toRead]));
					decompressed.reset(); //reset position to last mark
				}
				else {
					throw new AssertionError("bit should be 0 or 1, got " + i + ". Buffer position: " + compressed.position() + ", bit " + i + ".");
				}
			}
		}
